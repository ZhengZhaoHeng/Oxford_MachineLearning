...
for i = 1, iterations do
  ...
  if i % 10 == 0 then -- don't print *every* iteration, this is enough to get the gist
      ...
      print(string.format("minibatches processed: %6s, loss = %6.6f, test_loss = %6.6f", i, minibatch_loss[1], test_loss))
  end
...
gnuplot.plot({'Training',
  ...
  '-'}, {'Test', torch.linspace(1, #losses, #test_losses), torch.Tensor(test_losses), '~'})
gnuplot.plotflush()
...
-- caculate the test classification error
for i = 1, (#classPredictions)[1] do
	if classPredictions[i][1] ~= test.labels[i] then
		test_error = test_error + 1
	end
end
print(string.format("test classification error: %6.6f%%", test_error / test.data:size()[1] * 100))

-- caculate the train classification error
logProbs = model:forward(train.data)
classProbablities = torch.exp(logProbs)
_, classPredictions = torch.max(classProbablities, 2)
local train_error = 0;
for i = 1, (#classPredictions)[1] do
...
print(string.format("train classification error: %6.6f%%", train_error / train.data:size()[1] * 100))